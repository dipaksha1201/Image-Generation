...
# Import cfg from generate_images.py
from cfg import cfg

# Define GLU if it's used in the model
class GLU(nn.Module):
    def __init__(self):
        super(GLU, self).__init__()
...
class CA_NET(nn.Module):
    ...
    def reparametrize(self, mu, logvar):
        ...
        elif hasattr(torch, 'mps') and torch.backends.mps.is_available() and mu.device.type == 'mps':
            # For Apple Silicon MPS
            eps = torch.randn_like(std, device=mu.device)
        else:
            eps = torch.FloatTensor(std.size()).normal_()
            eps = eps.to(mu.device)  # Move to same device as mu
